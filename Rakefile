...
task :build => [:clean] do
  ...
  # minify src
  source = File.read('dist/jquery.confine.js')
  minified = Packr.pack(source, :shrink_vars => true, :base62 => false)
  header = /\/\*.*?\*\//m.match(source)

  # inject header
  File.open('dist/jquery.confine.min.js', 'w') do |combined|
...
task :release => [:build] do
  root = pwd+'/dist'
  zip_archive = pwd+'/dist/jquery.confine.zip'
...
task :test => [:build] do  
  begin
    # mac
    sh("open spec/jquery.confine.specs.html")
  rescue
    # windows
    sh("start spec/jquery.confine.specs.html")
  end
end
